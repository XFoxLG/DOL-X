...
- **上游更新决策（2026-08-10 修订）**：`vega` 分支已把 maplebirch 从 v4.1.13 升到 v4.1.14。
  真机单变量对照后退役了 basehead fallback 本地补丁，只保留桌宠重挂与 AU 换脸仪态两个补丁；
  构建不再引用第三方压缩局部名，也不再改写 `basehead.srcfn`。当前源代码的 AU-F 本地测试签名
  候选已完成构建、产物审计和 MuMu 12 抽样复验，完整套件为 269 passed；验证级别为
  `runtime-smoke-passed`，不代表正式签名或其余三个体型已上机。
  **该升级尚未构建发布，上面这版 0808 用的仍是 v4.1.13。**
  v4.1.14 解封了框架的 NPC 怀孕扩展，会向存档写入怀孕名单且没有回滚逻辑，
  因此升级后若退回 v4.1.13，存档里已写入的怀孕数据将无人处理。
- 上一代稳定版 `v0.5.10.12-1.0.8a-0713` 使用 maplebirch 3.x 栈，源码已存档在分支 `vega-archive-0713`，Release 仍保留可下载。
- 面向玩家的更新内容、下载选择与实测范围见 [`0808 Release 说明`](docs/release-notes/v0.5.10.12-1.0.8a-0808.md)。
  本版修复了开着桌宠切换脸型时桌宠头部消失、桌宠启用却不显示，以及 AU 版只切脸型后桌宠与
  角色预览表现不一致、闭眼或空白的问题，并会在读档时自动修好已经存进存档的错误组合。后者不是
  桌宠完全没有刷新：框架确实刷新了桌宠，但原入口传入了不存在的 AU 仪态，预览与桌宠的回退结果不同。
- 上一版 [`0804 Release 说明`](docs/release-notes/v0.5.10.12-1.0.8a-0804.md) 仍可查阅。
...
> - **AU 改脸的非法仪态组合已在 `-0808` 修复**：原因是切换脸型后仪态被设成了不存在的「默认」，导致角色预览可能靠 fallback 显示、桌宠却闭眼或空白。早先把这个根因笼统写成「AU 缺素材」是错的；但这也不等于已经证明 AU 每个合法路径的素材都完整，当前仍观察到部分 eyes/mouth 的内部失败请求。若它们形成可见红框或缺图，请保留存档和错误信息继续排查。脸红、流泪等细节妆效仍未完整验收。
