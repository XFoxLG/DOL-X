...
- 下一个版本 `v0.5.10.12-1.0.8a-0808` 已完成发版前验证但尚未发布，说明正文见
  [`0808 Release 说明`](docs/release-notes/v0.5.10.12-1.0.8a-0808.md)；它修复了 AU 换脸后仪态不同步、
  未再选择仪态时眼睛消失的问题。Releases 页面出现该 tag 后本节的「当前稳定版本」会同步更新。
...
> - **用 AU 改脸时可能弹出红色报错框，提示某张图片加载失败**：原因是切换脸型后仪态被置成了不存在的 `default`，于是游戏去找一套并不存在的眼睛图片。**AU 美化包的素材是齐的**，早先把原因写成"AU 缺素材"是错的。脸和表情实际仍能显示，不影响剧情和玩法，直接点 **Clear** 或 **Close** 关掉即可。该问题已在尚未发布的 `-0808` 中修复。脸红、流泪等细节妆效尚未完整验收。
