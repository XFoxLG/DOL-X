...
- **上游更新决策（2026-08-08 修订）**：`vega` 分支已把 maplebirch 从 v4.1.13 升到 v4.1.14，
  三个本地补丁在官方 4.1.14 资产上实测仍然成立（各写入一次标记、旧定位串归零、成员清单与 `boot.json` 字节不变）。
...
- 面向玩家的更新内容、下载选择与实测范围见 [`0808 Release 说明`](docs/release-notes/v0.5.10.12-1.0.8a-0808.md)。
  本版修复了开着桌宠切换脸型时桌宠头部消失、桌宠启用却不显示，以及 AU 版只切脸型时仪态不同步、
  眼睛消失的问题，并会在读档时自动修好已经存进存档的错误组合。
...
> - **AU 改脸的红框报错已在 `-0808` 修复**：原因是切换脸型后仪态被设成了不存在的「默认」，于是游戏去找一套并不存在的眼睛图片。**AU 美化包的素材是齐的**，早先把原因写成「AU 缺素材」是错的。如果你还在用 `-0804` 或更早的包，看到红框直接点 **Clear** 或 **Close** 关掉即可，不影响剧情和玩法。脸红、流泪等细节妆效仍未完整验收。
