...
| Feature | Asset | 版本 | 状态 |
|---------|-------|------|------|
| `au-f` | `AUfemale.model_v0.9.3.zip` | v0.9.3 | 当前诊断锁定版本；与上游 Lyra 和 AOKIUTAGE 最新 model asset 对齐 |
...
1. Base 无 AU (`5218560`) 作为对照组。
2. AU-F (`5219584`) 使用 v0.9.3 且 NeoUI 禁用，验证侧边栏是否恢复正常。
3. AU-M (`5220608`) 与 AU-A (`5222656`) 分别用最新构建复测，判断问题是否只影响 AU-F。
4. 如 AU-F 仍出错，再构建 AU-F without Mae's Picvary / NPC Avatars 诊断包。
5. 暂不默认切换到 `imgpack`；只有 model 方式的 UI/mod 隔离用尽后再单独测试。
