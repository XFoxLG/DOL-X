...
> "框架要求" 列一律指**框架 mod 版本**。
>
> 当前稳定 Release `v0.5.10.12-1.0.8a-0808` 与 `vega` 均使用 4.x 栈；0713 是保留的历史
> 3.x 回滚版本。
...
|----------|------|-----------------|----------|----------|----------|------|
| maplebirch Framework | **v4.1.14** | - | 0.5.10.12 | ⚠️ 已升级，待真机 | 怀孕扩展解封后完全未实测；全功能未遍历；云存档需自建后端 | 2026-08-08 从 v4.1.13 升级。官方资产 186903 B / `e44c9aed…`，digest 与 GitHub 一致；三个本地补丁在真实资产上实测仍成立。此前"不混入 4.1.14"的决策已推翻 |
...
**文档状态**: 当前稳定版为 `v0.5.10.12-1.0.8a-0808`，发版前全四码 CI 构建与 8 件产物静态审计已验证。
